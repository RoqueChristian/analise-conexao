...
try:
    ...
    print(f"Aviso: Não foi possível inicializar o cliente Oracle. Certifique-se de que o caminho está correto ou que não é necessário: {e}")
...
sql_conexao_clientes_ol = """
    WITH fornecedor_pedido AS (
        SELECT
            m.numped,
            MAX(f.cgc) as cgc_fornecedor,
            MAX(f.fornecedor) as nome_fornecedor
        FROM pcmov m
